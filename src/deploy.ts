/**
 * Deploy Kompers to Midnight Preview (default) or another network via --network.
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║ Deploy kompers to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  if (network === 'undeployed' && balance === 0n) {
    console.error('\n❌ Genesis-seed wallet has zero NIGHT. Check docker compose logs.\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const errCause = err instanceof Error && err.cause ? String(err.cause) : '';
  ...
  console.log(' Saved to .midnight-state.json (gitignored)\n');
